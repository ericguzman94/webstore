...
public partial class Pages_ShoppingCart : Page
{
    ...
    private void GetPurchasesInCart()
    {
        ...
        // generate HTML
        List<Cart> purchaseList = model.GetOrdersInCart();
       CreateShopTable(purchaseList, out subTotal);

        //Add totals to webpage
        double tax = subTotal * 0.09;
        double totalAmount = subTotal + tax + 5;

        //Dispplay values on webpage
    ...
    private void CreateShopTable(List<Cart> purchaseList, out double subTotal)
    {
        ...
        foreach (Cart cart in purchaseList)
        {
            ...
            TableCell cell2_4 = new TableCell { Text = "$ " + Math.Round(Convert.ToDouble(cart.Amount * product.Price), 2) };
            ...
            double aa = Convert.ToDouble(cart.Amount * product.Price);
            subTotal += aa;
        }
    }
    ...
    private string GeneratePaypalButton(double subTotal)
    {
        ...
        string paypal = string.Format(
            @"<script async='async' src='https://www.paypalobjects.com/js/external/paypal-button.min.js?merchant=striderkills@gmail.com' 
                data-button='buynow' 
